...
--
-- TODO: Is this still relevant now that chain sync no longer deals with
-- clock skew?
prop_genUpdateSchedule_notInFuture :: SecurityParam -> Property
prop_genUpdateSchedule_notInFuture securityParam =
    forAll genUpdateSchedule' $ \updates -> conjoin
      [ chainUpdateHighestSlotNo chainUpdate <= slot
      | (slot, chainUpdates) <- Map.toAscList updates
      , chainUpdate          <- chainUpdates
      ]
  where
    genUpdateSchedule' = evalStateT
      (genUpdateSchedule securityParam)
      emptyUpdateState
